...
# ── 60 : Physiological Switch ────────────────────────────────────────────────

def plot_physiological_switch():
    """
    Hero chart for the thesis: R² vs. sub-max test duration, showing the
    dramatic jump from 30s sprint (R²=0.516) to 2-min effort (R²=0.704)
    as the 'physiological switch' from neuromuscular to aerobic dominance.

    Reads outputs/results/submax_threshold.csv (generated by submax_threshold.py).
    Sprint baseline (R²=0.516) is prepended as the leftmost point.
    """
    from pipeline.config import OUTPUT_RESULTS

    csv_path = OUTPUT_RESULTS / "submax_threshold.csv"
    if not csv_path.exists():
        print("  [plots] submax_threshold.csv not found — skipping switch plot.")
        return None

    df = pd.read_csv(csv_path)

    # Build unified x-axis: sprint baseline + 6 sub-max durations
    x_labels = ["30s Sprint", "2 min", "3 min", "5 min", "8 min", "10 min", "30 min"]
    x_pos    = list(range(len(x_labels)))

    # Sprint baseline from pipeline results (log-target, sprint_bio)
    SPRINT_R2 = 0.516
    SPRINT_MAE = 33.7

    r2_vals  = [SPRINT_R2]  + list(df["R2_alone"])
    mae_vals = [SPRINT_MAE] + list(df["MAE_alone"])

    THRESHOLD = 0.70
    SWITCH_X  = 1          # index of 2-min point

    fig, ax = plt.subplots(figsize=(11, 6))

    # ── Shaded zones ────────────────────────────────────────────────────────
    ax.axvspan(-0.5, SWITCH_X, alpha=0.07, color="tomato",    zorder=0)
    ax.axvspan(SWITCH_X, 6.5,  alpha=0.07, color="mediumseagreen", zorder=0)

    # Zone labels
    ax.text(0.22, 0.36, "Neuromuscular\nZone",
            transform=ax.transAxes, ha="center", va="center",
            fontsize=11, color="tomato", alpha=0.75, fontweight="bold")
    ax.text(0.68, 0.36, "Aerobic Zone",
            transform=ax.transAxes, ha="center", va="center",
            fontsize=11, color="mediumseagreen", alpha=0.75, fontweight="bold")

    # ── Threshold line ───────────────────────────────────────────────────────
    ax.axhline(THRESHOLD, color=CLR_ACCENT, linestyle="--", linewidth=1.6,
               zorder=2, label=f"R² = {THRESHOLD} threshold")

    # ── Vertical switch line ─────────────────────────────────────────────────
    ax.axvline(SWITCH_X, color="gray", linestyle=":", linewidth=1.4, zorder=2)

    # ── Main curve ───────────────────────────────────────────────────────────
    ax.plot(x_pos[1:], r2_vals[1:],
            color=CLR_PRIMARY, linewidth=2.5, marker="o", markersize=8,
            zorder=4, label="Sub-max R² (XGBoost)")

    # ── Sprint baseline point ────────────────────────────────────────────────
    ax.scatter([0], [SPRINT_R2],
               color=CLR_ACCENT, marker="X", s=140, zorder=5,
               label=f"30s sprint baseline (R²={SPRINT_R2})")
    ax.annotate(f"R²={SPRINT_R2}",
                xy=(0, SPRINT_R2), xytext=(0.22, SPRINT_R2 - 0.04),
                fontsize=9, color=CLR_ACCENT, ha="center")

    # ── Annotate each sub-max point ──────────────────────────────────────────
    for xi, (r2, mae) in zip(x_pos[1:], zip(r2_vals[1:], mae_vals[1:])):
        ax.annotate(f"R²={r2:.3f}\n±{mae:.0f}W",
                    xy=(xi, r2), xytext=(xi, r2 + 0.022),
                    ha="center", va="bottom", fontsize=7.5, color="dimgray")

    # ── Switch callout ───────────────────────────────────────────────────────
    ax.annotate(
        "⚡ Physiological Switch\nR² crosses 0.70 at 2 min\n(+18.8% vs sprint alone)",
        xy=(SWITCH_X, r2_vals[SWITCH_X]),
        xytext=(SWITCH_X + 1.1, 0.645),
        fontsize=9.5, fontweight="bold", color="black",
        arrowprops=dict(arrowstyle="->", color="black", lw=1.3),
        bbox=dict(boxstyle="round,pad=0.4", fc="lightyellow", ec="goldenrod", lw=1.2),
    )
    ...
    ax.set_ylabel("R²  (FTP Prediction Accuracy)", fontsize=12)
    ax.set_xlabel("Sub-Maximal Test Duration", fontsize=12)
    ax.set_title(
        "Sub-Maximal Test Duration vs. FTP Prediction Accuracy\n"
        "XGBoost · n = 4,768 cyclists · Test set R² (30/70 stratified split)",
    ...
    plt.tight_layout()
    return _save(fig, "60_physiological_switch.png")
